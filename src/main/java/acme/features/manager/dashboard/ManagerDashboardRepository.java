...
public interface ManagerDashboardRepository extends AbstractRepository {
	...
	@Query("select 1.0 * count(l) / nullif((select count(lg) from Leg lg where lg.flight.manager.id = :managerId),0) from Leg l " + //
		"where l.flight.manager.id = :managerId and l.status = :status")
	Double findRatioStatusLegs(int managerId, LegStatus status);

	@Query("select a from Airport a where a in (select o from Leg l join l.origin o where l.flight.manager.id = :managerId) or a in " + //
		"(select d from Leg lg join lg.destination d where lg.flight.manager.id = :managerId) group by a order by " + //
		"((select count(l) from Leg l where l.flight.manager.id = :managerId and l.origin.id = a.id) " + //
		"+ (select count(lg) from Leg lg where lg.flight.manager.id = :managerId and lg.destination.id = a.id)) desc")
	List<Airport> findMostPopularAirport(int managerId, PageRequest pageRequest);

	@Query("select a from Airport a where a in (select o from Leg l join l.origin o where l.flight.manager.id = :managerId) or a in " + //
		"(select d from Leg lg join lg.destination d where lg.flight.manager.id = :managerId) group by a order by " + //
		"((select count(l) from Leg l where l.flight.manager.id = :managerId and l.origin.id = a.id) " + //
		"+ (select count(lg) from Leg lg where lg.flight.manager.id = :managerId and lg.destination.id = a.id)) asc")
