...
@GuiService
public class AdministratorSystemConfigurationUpdateService extends AbstractGuiService<Administrator, SystemConfig> {
	...
	public void validate(final SystemConfig systemConfig) {
		...
		if (scs.contains(systemConfig) && scs.size() < 2)
			super.state(false, "*", "acme.validation.currency.is-only-system-currency");
